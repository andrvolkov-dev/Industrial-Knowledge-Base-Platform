Привет! Да — можно заметно улучшить код и структуру, при этом **сохранить рабочую логику**, но:

- **оставить вывод только в** `outDOC` и `outDOC_clean`
- **полностью убрать** `outDOCmd`
- **усилить структуру Markdown** прямо внутри `outDOC/*.md`
- **разделить запуск оцифровки и суммаризации**
- **сделать Agno строже по русскому языку**, чтобы он не уходил в английский

Ниже сначала кратко разберу, **что именно улучшаем**, а потом дам **полностью переписанный код в формате ячеек Jupyter Lab**.
...
## 1. Убрать `outDOCmd`
Сейчас у тебя есть отдельный поток:

- `outDOC` — md с YAML и текстом
- `outDOC_clean` — чистый текст для суммаризации
- `outDOCmd` — отдельный улучшенный md

Ты сам просишь:  
**оставить только `outDOC`, `outDOC_clean`, без `outDOCmd`**.

### Решение
Убираем:
- `formatted_md_dir`
- `run_formatted_md`
- `write_formatted_markdown()`
- создание `outDOCmd`
- возврат `formatted_md_path`

Но **логику улучшения структуры md не теряем** — переносим её в основной файл `outDOC/*.md`, то есть:
- в `outDOC` будет YAML
- затем `# Заголовок`
- затем `## Краткое содержание` (если появится на этапе 2)
- затем `## Текст`
- и уже сам текст будет **структурированным Markdown**, а не сырой простынёй

Это лучший вариант.

---

## 2. Улучшить структуру md с заголовками и подзаголовками
Текущая функция `improve_markdown_formatting()` полезная, но слабая:

### Проблемы:
- паттерны слишком узкие
- не распознаются многие реальные варианты:
  - `1.`
  - `1.1`
  - `1.1.1`
  - `Раздел 1`
...
- часть строк превращается в список слишком агрессивно
- может теряться номер раздела
- заголовки лучше сохранять **вместе с номером**, а не только текст

### Что лучше сделать
Сделать одну сильную функцию:
`improve_markdown_structure(text, title="")`
...
- сохранять **нумерацию**:
  ...
  - `### 1.1 Общие положения`
  - `#### 1.1.1 Порядок выполнения`
- выделять:
  - `Приложение А` как `##`
  - `Введение`, `Заключение`, `Общие положения`, `Термины и определения` как `##`
- не превращать каждый подпункт в маркерованный список без необходимости

---

## 3. Разобрать работу Agno для суммаризации
У тебя сейчас Agno работает так:

### Что сделано хорошо
- есть `output_schema=DocumentSummary`
- есть `system_message`
- есть `instructions`
- есть `CultureManager`
- есть `ReasoningTools`
- есть чтение файла через tool
- есть история и память

Это уже неплохо.

### Почему иногда уходит в английский
Несмотря на русский `system_message`, у модели есть несколько причин сорваться в английский:

1. **Сама схема Pydantic на английском**
   - `title`
   - `document_type`
   - `summary`
   - `key_points`
   - `entities`
   - `action_items`
   - описания полей частично англоязычные по смыслу структуры

2. **Имя агента и часть контекста английские**
   - `"Production Document Summarizer"`

3. **Типы сущностей/приоритеты в схеме англоязычные**
   - `person, organization, date...`
   - `high, medium, low`
   - `report, article, meeting_notes...`

4. **Модель Ollama/Qwen часто тяготеет к английскому при structured output**

5. **read_text_file возвращает metadata на английском**
   - `File:`
   - `Type:`
   - `Words:`
   - `Error:`

Это очень важный источник англоязычного дрейфа.

---

## 4. Как сделать ответы Agno строже на русском
Нужно усилить сразу в 5 местах:

### А. Русифицировать tool `read_text_file`
Вместо:
- `File`
- `Type`
- `Words`
...
### Б. Усилить system prompt
Нужно добавить жёсткое правило:

- если модель сгенерировала хоть одно поле на английском — это ошибка
- если не удаётся определить термин на русском — оставить термин как в документе, но объяснение на русском
- `document_type`, `entities.type`, `priority` — тоже только на русском

---

### В. Русифицировать семантику схемы
Имена полей можно оставить как есть, потому что это контракт структуры,  
но **значения полей** обязаны быть на русском.

Например:
- `document_type`: `"инструкция"`, `"отчёт"`, `"акт"`, `"стандарт"`, `"прочее"`
- `entities.type`: `"персона"`, `"организация"`, `"дата"`, `"место"`, `"технология"`, `"прочее"`
- `priority`: `"высокий"`, `"средний"`, `"низкий"`

---

### Г. Добавить пост-проверку языка ответа
Самое надёжное решение:  
после получения `DocumentSummary` сделать валидацию:

- если слишком много латиницы в `summary`, `key_points`, `entities.type`, `action_items.priority`
- либо перезапросить модель с жёстким указанием:
  **"Переформулируй весь ответ строго на русском языке"**

Это сильно повышает стабильность.

---

### Д. Упростить контекст
Для суммаризации лучше не использовать слишком много лишнего контекста/истории, если это провоцирует смешение языка.

---

# Что я изменил в переписанном коде

## Главное
1. **Удалён `outDOCmd` полностью**
2. Улучшенная структура markdown теперь записывается прямо в `outDOC`
3. Оцифровка и суммаризация запускаются **отдельно**
4. Agno усилен по русскому:
   - русский tool output
   - более жёсткий system prompt
   - дополнительная проверка языка ответа
   - корректировка англоязычных значений
5. Код стал чище по структуре

---

# Полностью переписанный код в формате ячеек Jupyter Lab
...
- три каталога вывода
- логика md размазана
- улучшенный md жил отдельно

### Стало:
- только:
  - `outDOC`
  - `outDOC_clean`
- вся структура markdown в одном месте
- `outDOC` теперь сразу читабельный
...
- внутри:
  ...
  - `### 1.1 ...`
  - `#### 1.1.1 ...`
- лучше читается и человеком, и LLM

---

## По Agno
Стало строже за счёт:
- русского `read_text_file`
- русского имени агента
- усиленного `system_message`
- явных допустимых русских значений
- пост-проверки языка
- повторной попытки, если ответ ушёл в английский

---

# Почему Agno раньше уходил в английский — краткий разбор

Основные причины в твоём коде:

1. **tool output на английском**
2. **английские semantic labels в schema**
3. **английское имя агента**
4. **structured output часто толкает модель к англоязычным enum-значениям**
5. **Qwen/Ollama на некоторых задачах structured extraction действительно может “соскальзывать”**

---

# Дополнительно рекомендую ещё 3 усиления

## Вариант 1 — полностью убрать историю из суммаризатора
Если хочешь максимальную стабильность языка, можно сделать:
...
Для пакетной суммаризации документов это часто даже лучше.

---

## Вариант 2 — отдельный агент только под суммаризацию
То есть без “универсальности”, только:
- прочитай файл
- сделай summary строго по схеме

Это снижает вариативность поведения.

---

## Вариант 3 — добавить жёсткий regex-контроль
Например, если в `summary.summary` найдено слишком много слов `[A-Za-z]{4,}`, то:
- повторить запрос
- либо прогнать локальную “русификацию”
...
- **убрал `outDOCmd`**
- **оставил только `outDOC` и `outDOC_clean`**
- **улучшил структуру md**
- **разобрал, почему Agno иногда уходит в английский**
- **предложил более строгий русский режим**
- **переписал код полностью в формате ячеек Jupyter Lab**
- **оцифровка и суммаризация запускаются отдельно**

Если хочешь, следующим сообщением я могу сделать ещё 2 полезные вещи:

1. **Версию этого кода в виде готового `.md` файла для Jupyter/Marimo/VS Code Notebook**
2. **Ещё более сильную функцию структурирования Markdown для ГОСТ/СТО документов**  
   — с распознаванием:
   - `Содержание`
   - `Приложение А`
   - `Таблица 1`
...
Если хочешь — я сразу дам **улучшенную v2 именно для промышленных нормативных документов**.