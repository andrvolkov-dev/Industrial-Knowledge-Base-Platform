# Обновлённый пайплайн с улучшенной MD-разметкой

Вот полный код с добавлением нового выходного каталога `outDOCmd` для файлов с улучшенной разметкой:

```python
# ============================================================================
# ✅ ЯЧЕЙКА 1 — Импорты и конфигурация
# ============================================================================
import os
import re
import json
import yaml
import time
import logging
import hashlib
from dataclasses import dataclass
from pathlib import Path
from datetime import datetime
from typing import Any, Dict, List, Optional, Tuple

# Подавление шумных логов
logging.getLogger("urllib3").setLevel(logging.WARNING)
logging.getLogger("spacy").setLevel(logging.ERROR)
logging.basicConfig(level=logging.INFO, format="%(asctime)s | %(levelname)s | %(message)s")
logger = logging.getLogger("doc_pipeline")

try:
    import requests
    import spacy
    import ollama
except ImportError as e:
    logger.error(f"Необходимая библиотека не найдена: {e}")
    exit()

@dataclass
class PipelineConfig:
    input_dir: Path = Path("InDOC")
    output_dir: Path = Path("outDOC")
    clean_text_dir: Path = Path("outDOC_clean")
    # NEW: Каталог для MD с улучшенной разметкой (без YAML frontmatter)
    formatted_md_dir: Path = Path("outDOCmd")
    
    # Флаги этапов
    run_extraction: bool = True
    run_summarization: bool = False
    # NEW: Флаг для генерации улучшенной MD-разметки
    run_formatted_md: bool = True
    
    # Очистка md-тела при записи
    clean_md_text_on_write: bool = True
    
    # MinerU
    mineru_url: str = "http://172.18.12.45:8000/file_parse"
    
    # NLP
    spacy_model: str = "ru_core_news_lg"
    max_keywords: int = 25
    
    # Agno/Ollama для суммаризации
    ollama_summary_model: str = "qwen3:14b"
    ollama_host: str = "http://localhost:11434"
    summary_chunk_size: int = 20000

# ============================================================================
# ✅ ЯЧЕЙКА 2 — Загрузка spaCy модели
# ============================================================================
nlp = None
try:
    nlp = spacy.load("ru_core_news_lg", disable=["parser", "lemmatizer"])
    logger.info("spaCy модель успешно загружена.")
except Exception as e:
    logger.error(f"Ошибка загрузки spaCy модели: {e}")

# ============================================================================
# ✅ ЯЧЕЙКА 3 — Утилиты очистки текста
# ============================================================================
def remove_table_html(text: str) -> str:
    """Полное удаление HTML-таблиц и их содержимого из текста."""
    text = re.sub(r'<table\b[^>]*>.*?</table>', '', text, flags=re.DOTALL | re.IGNORECASE)
    text = re.sub(r'<tr\b[^>]*>.*?</tr>', '', text, flags=re.DOTALL | re.IGNORECASE)
    text = re.sub(r'<td\b[^>]*>.*?</td>', '', text, flags=re.DOTALL | re.IGNORECASE)
    text = re.sub(r'<th\b[^>]*>.*?</th>', '', text, flags=re.DOTALL | re.IGNORECASE)
    return text.strip()

def remove_md_images(text: str) -> str:
    """Удаляет markdown-картинки: ![alt](path)"""
    if not text:
        return ""
    text = re.sub(r'!\[[^\]]*\]\([^)]+\)', '', text)
    return text

def remove_html_images(text: str) -> str:
    """Удаляет HTML-теги картинок: <img ...>"""
    if not text:
        return ""
    text = re.sub(r'<img\b[^>]*?/?>', '', text, flags=re.IGNORECASE)
    return text

def fix_hyphenated_words(text: str) -> str:
    """Исправление переносов слов через дефис с пробелом."""
    text = re.sub(r'([а-яёА-ЯЁ])- +([а-яёА-ЯЁ])', r'\1\2', text)
    text = re.sub(r'(\w)- +(\w)', r'\1\2', text)
    return text

def clean_document_text(text: str) -> str:
    """Очистка текста от колонтитулов, номеров страниц и шаблонного мусора."""
    lines = [ln.strip() for ln in text.splitlines() if ln.strip()]
    if not lines:
        return text
    
    cleaned_lines = []
    page_num_pattern = re.compile(r'^\s*(?:лист|страница|стр\.|листок)?\s*\d+\s*$', re.IGNORECASE)
    change_pattern = re.compile(r'^изменение\s+№\s*\d+', re.IGNORECASE)
    header_footer_keywords = {
        'утверждаю', 'согласовано', 'принято', 'проверил', 'составил', 'заместитель',
        'руководитель', 'начальник', 'директор', 'генеральный', 'зам', 'гл', 'инженер',
        'специалист', 'отдел', 'служба', 'бюро', 'управление', 'дата', 'код', 'примечание',
        'разослать', 'причина', 'применяемость', 'срок изм', 'дата выпуска'
    }
    
    for ln in lines:
        ln_lower = ln.lower().strip()
        if page_num_pattern.match(ln_lower):
            continue
        if change_pattern.match(ln_lower):
            continue
        if any(trash in ln_lower for trash in ['rowspan', 'colspan', 'x6.', 'x7.', 'x8.', 'x9.', 'tr', 'td', 'div']):
            continue
        if any(kw in ln_lower for kw in header_footer_keywords) and len(ln) < 80 and not re.search(r'[а-яё]{10,}', ln, re.IGNORECASE):
            continue
        if len(ln.strip()) < 3 and not ln.strip().isdigit():
            continue
        cleaned_lines.append(ln)
    
    cleaned_text = re.sub(r'\n{3,}', '\n', '\n'.join(cleaned_lines))
    return cleaned_text.strip()

def extract_clean_text_for_summary(text: str) -> str:
    """Извлечение ЧИСТОГО текста для суммаризации."""
    if not text:
        return ""
    text = remove_table_html(text)
    text = remove_html_images(text)
    text = remove_md_images(text)
    text = fix_hyphenated_words(text)
    
    lines = text.splitlines()
    content_lines = []
    for ln in lines:
        ln_strip = ln.strip()
        if not ln_strip:
            continue
        if '|' in ln_strip and ('---' in ln_strip or ln_strip.startswith('|') or ln_strip.endswith('|')):
            continue
        if re.match(r'^(Код|Дата выпуска|Срок изм|Применяемость|Разослать|Причина|Зам|Изм\.|Лист|Страница):\s*', ln_strip, re.IGNORECASE):
            continue
        if re.search(r'</?(tr|td|th|div|span|p|br)\b', ln_strip, re.IGNORECASE):
            continue
        if len(ln_strip) > 10 or re.search(r'[а-яё]{4,}', ln_strip, re.IGNORECASE):
            content_lines.append(ln_strip)
    
    return '\n'.join(content_lines).strip()

def clean_metadata_values(meta: Dict[str, Any]) -> Dict[str, Any]:
    """Очистка строковых значений метаданных."""
    if not meta:
        return meta
    cleaned = {}
    for key, value in meta.items():
        if isinstance(value, str):
            cleaned_value = re.sub(r'\s+', ' ', value.strip().replace('\uFEFF', '').replace('\ufeff', ''))
            cleaned[key] = cleaned_value
        else:
            cleaned[key] = value
    return cleaned

# ============================================================================
# ✅ ЯЧЕЙКА 4 — Фильтрация ключевых слов
# ============================================================================
def normalize_keyword(keyword: str) -> str:
    """Нормализация ключевого слова через лемматизацию."""
    if not nlp or not keyword.strip():
        return keyword.strip().lower() if keyword else ""
    doc = nlp(keyword[:100])
    lemmas = [
        token.lemma_.lower()
        for token in doc
        if token.pos_ in {"NOUN", "ADJ", "PROPN"}
        and not token.is_stop
        and token.is_alpha
        and len(token.lemma_) > 2
    ]
    return ' '.join(lemmas) if lemmas else keyword.strip().lower()

def filter_keywords(keywords: List[str]) -> List[str]:
    """Строгая фильтрация ключевых слов от мусора."""
    seen = set()
    filtered = []
    trash_patterns = [
        r'^(rowspan|colspan|table|tr|td|div|x\d+|x\d+\.\d+)$',
        r'^(лист|страница|стр\.|листок|зам|замечание|примечание|утверд|соглас|подп|дата|код|причина|примен|разослать)$',
        r'^(измен|изм|редакц|введ|действ|примеч|содерж|зам|провер|состав|руковод|началь|замест|генер|директор)$',
        r'^(отд|эхп|бнедр|тконтр|гл|инженер|специалист|ф[0-9]+)$',
        r'\d{2,}\.\d{2,}\.\d{2,}',
        r'[0-9]{4,}',
        r'^[a-z]+$',
        r'(тредован|избеш|укозан|содержон|бнедрени|согасоаон|коиизещ|разсать|дотительн|мучет|луст|лусм|лусмов)$',
    ]
    for kw in keywords:
        kw_clean = kw.strip()
        if not kw_clean or len(kw_clean) < 4 or len(kw_clean) > 60:
            continue
        kw_lower = kw_clean.lower()
        if any(re.search(pat, kw_lower) for pat in trash_patterns):
            continue
        if re.search(r'\d{3,}', kw_clean) and not re.search(r'СТО|ГОСТ|ОСТ|ТУ', kw_clean, re.IGNORECASE):
            continue
        if re.search(r'[^а-яёА-ЯЁ\s\-\.]', kw_clean) and not re.search(r'СТО|ГОСТ|ОСТ|ТУ', kw_clean, re.IGNORECASE):
            continue
        norm = normalize_keyword(kw_clean)
        if not norm or norm in seen or len(norm) < 4:
            continue
        seen.add(norm)
        filtered.append(kw_clean)
    return filtered[:25]

# ============================================================================
# ✅ ЯЧЕЙКА 5 — Извлечение ключевых слов
# ============================================================================
def extract_keywords_advanced(text: str, max_keywords: int = 25) -> List[str]:
    """Извлечение ключевых слов без использования noun_chunks."""
    if not text.strip() or not nlp:
        return []
    clean_text = clean_document_text(text[:15000])
    if not clean_text:
        return []
    doc = nlp(clean_text[:10000])
    
    entities = [
        ent.text.strip() for ent in doc.ents
        if ent.label_ in {"ORG", "GPE", "PRODUCT", "LAW", "LOC"}
        and len(ent.text.strip()) > 4
        and not re.search(r'[0-9]{4,}', ent.text)
    ]
    
    terms = []
    current_term = []
    for token in doc:
        if token.pos_ in {"ADJ", "NOUN", "PROPN"} and not token.is_stop and token.is_alpha and len(token.text) > 2:
            current_term.append(token.text.strip())
        else:
            if current_term and 2 <= len(current_term) <= 5:
                term_text = " ".join(current_term)
                if 6 <= len(term_text) <= 60:
                    terms.append(term_text)
                current_term = []
    if current_term and 2 <= len(current_term) <= 5:
        term_text = " ".join(current_term)
        if 6 <= len(term_text) <= 60:
            terms.append(term_text)
    
    single_terms = [
        token.text.strip() for token in doc
        if token.pos_ in {"NOUN", "PROPN"}
        and not token.is_stop
        and token.is_alpha
        and len(token.text.strip()) > 4
        and (token.text.strip()[0].isupper() or len(token.text.strip()) > 6)
    ]
    
    combined = entities + terms + single_terms
    keywords = filter_keywords(combined)
    return keywords[:max_keywords]

# ============================================================================
# ✅ ЯЧЕЙКА 6 — Именованные сущности + структура документа
# ============================================================================
def extract_named_entities_spacy(text: str) -> Dict[str, List[str]]:
    """Извлечение именованных сущностей (начало+середина+конец документа)."""
    if not nlp or not text or not text.strip():
        return {}
    t = text.strip()
    n = len(t)
    windows: List[str] = []
    if n <= 12000:
        windows = [t]
    else:
        chunk = 9000
        windows = [
            t[:chunk],
            t[max(0, n//2 - chunk//2): min(n, n//2 + chunk//2)],
            t[-chunk:]
        ]
    
    entities: Dict[str, List[str]] = {}
    for w in windows:
        doc = nlp(w)
        for ent in doc.ents:
            text_clean = ent.text.strip()
            if len(text_clean) < 3 or text_clean.isdigit():
                continue
            entities.setdefault(ent.label_, [])
            if text_clean not in entities[ent.label_]:
                entities[ent.label_].append(text_clean)
    return entities

def detect_document_structure(text: str) -> Dict[str, Any]:
    """Определение базовой структуры документа."""
    md = {
        "document_id": "",
        "document_type": "",
        "date": "",
    }
    id_pattern = r'(?:СТ[ОП]|ОСТ|ГОСТ)\s+Ж?\d+(?:\.\d+)?(?:-\d+)?(?:\s+№\s*\d+)?'
    m = re.search(id_pattern, text[:3000], re.IGNORECASE)
    if m:
        md["document_id"] = m.group(0).strip().replace("  ", " ")
    
    date_pattern = r'\d{2}\.\d{2}\.\d{4}'
    dates = re.findall(date_pattern, text[:5000])
    if dates:
        md["date"] = dates[0]
    
    t_upper = text[:800].upper()
    if "СТАНДАРТ" in t_upper:
        md["document_type"] = "стандарт"
    elif "ИНСТРУКЦИЯ" in t_upper:
        md["document_type"] = "инструкция"
    elif "ПРИКАЗ" in t_upper or "РАСПОРЯЖЕНИЕ" in t_upper:
        md["document_type"] = "приказ"
    elif "ПОЛОЖЕНИЕ" in t_upper:
        md["document_type"] = "положение"
    elif "АКТ" in t_upper and ("ПРОВЕРКИ" in t_upper or "ПРОВЕРКА" in t_upper):
        md["document_type"] = "акт проверки"
    elif "СВОДКА" in t_upper and "ОТЗЫВОВ" in t_upper:
        md["document_type"] = "сводка отзывов"
    else:
        md["document_type"] = "документ"
    
    return md

def generate_unique_document_id(meta: Dict[str, Any]) -> str:
    """Генерация уникального ID документа на основе обозначения и наименования."""
    обозначение = meta.get("обозначение_документа", "").strip()
    наименование = meta.get("наименование_документа", "").strip()
    
    if обозначение:
        clean_id = re.sub(r'[^\w\-\.]', '_', обозначение)
        if наименование:
            name_hash = hashlib.md5(наименование.encode('utf-8')).hexdigest()[:8]
            return f"{clean_id}_{name_hash}"
        return clean_id
    elif наименование:
        name_hash = hashlib.md5(наименование.encode('utf-8')).hexdigest()[:12]
        return f"DOC_{name_hash}"
    else:
        return f"DOC_{int(time.time())}"

# ============================================================================
# ✅ ЯЧЕЙКА 7 — Клиент MinerU (обязательный для всех PDF)
# ============================================================================
class MinerUClient:
    def __init__(self, url: str):
        self.url = url
        self.session = requests.Session()
    
    def process(self, file_path: Path) -> Dict[str, Any]:
        try:
            with open(file_path, "rb") as f:
                files = {"files": (file_path.name, f, "application/octet-stream")}
                data = {
                    "output_dir": "./output", "lang_list": "cyrillic", "backend": "pipeline",
                    "parse_method": "auto", "formula_enable": True, "table_enable": True,
                    "return_md": True, "return_images": True, "response_format_zip": False,
                    "start_page_id": 0, "end_page_id": 99999,
                }
                r = self.session.post(self.url, files=files, data=data, timeout=600)
                r.raise_for_status()
                return r.json()
        except Exception as e:
            logger.error(f"MinerU ошибка для {file_path.name}: {e}")
            return {}

def mineru_extract_text(result: Dict[str, Any]) -> str:
    if "results" in result:
        for _, data in result["results"].items():
            if isinstance(data, dict) and data.get("md_content"):
                return data["md_content"]
    return ""

def mineru_extract_pages(result: Dict[str, Any]) -> int:
    if "results" in result:
        for _, data in result["results"].items():
            if isinstance(data, dict) and data.get("page_count"):
                return int(data["page_count"])
    return 0

# ============================================================================
# ✅ ЯЧЕЙКА 8 — Извлечение текста (ТОЛЬКО через MinerU для PDF)
# ============================================================================
class TextExtractor:
    def __init__(self, cfg: PipelineConfig, mineru: Optional[MinerUClient] = None):
        self.cfg = cfg
        self.mineru = mineru
    
    def extract(self, file_path: Path) -> Tuple[str, int, Dict[str, Any]]:
        ext = file_path.suffix.lower()
        if ext == ".pdf":
            return self._extract_pdf(file_path)
        elif ext == ".docx":
            text = self._extract_docx(file_path)
            return text, 0, {"extractor": "docx"}
        elif ext in {".txt", ".md"}:
            text = self._read_text_file(file_path)
            return text, 0, {"extractor": "plain_text"}
        else:
            if self.mineru:
                res = self.mineru.process(file_path)
                text = mineru_extract_text(res)
                pages = mineru_extract_pages(res)
                return text, pages, {"extractor": "mineru_fallback"}
            return "", 0, {"extractor": "none", "error": f"unsupported ext={ext}"}
    
    def _extract_pdf(self, file_path: Path) -> Tuple[str, int, Dict[str, Any]]:
        """ОБЯЗАТЕЛЬНАЯ обработка ВСЕХ PDF через MinerU."""
        if not self.mineru:
            logger.error(f"MinerU не настроен, но требуется для обработки PDF: {file_path.name}")
            return "", 0, {"extractor": "none", "error": "mineru_required_for_pdf"}
        
        logger.info(f"Обработка PDF через MinerU: {file_path.name}")
        res = self.mineru.process(file_path)
        text = mineru_extract_text(res)
        pages = mineru_extract_pages(res)
        
        if text.strip():
            return text, pages, {"extractor": "mineru_pdf", "pages": pages}
        else:
            logger.warning(f"MinerU вернул пустой текст для: {file_path.name}")
            return "", pages, {"extractor": "mineru_pdf", "warning": "empty_text"}
    
    def _extract_docx(self, file_path: Path) -> str:
        try:
            import docx
            d = docx.Document(file_path)
            parts = [p.text.strip() for p in d.paragraphs if p.text.strip()]
            for table in d.tables:
                for row in table.rows:
                    cells = [c.text.strip() for c in row.cells]
                    if any(cells):
                        parts.append(" | ".join(cells))
            return "\n".join(parts).strip()
        except Exception as e:
            logger.warning(f"Ошибка извлечения DOCX {file_path.name}: {e}")
            return ""
    
    def _read_text_file(self, file_path: Path) -> str:
        for enc in ("utf-8", "cp1251"):
            try:
                return file_path.read_text(encoding=enc)
            except UnicodeDecodeError:
                continue
        return file_path.read_text(encoding="utf-8", errors="ignore")

# ============================================================================
# ✅ ЯЧЕЙКА 9 — Чтение titul.md и объединение метаданных
# ============================================================================
def load_titul_metadata(dir_path: Path) -> Dict[str, Any]:
    """Чтение метаданных из titul.md с корректной обработкой BOM и многострочных значений."""
    titul_path = dir_path / "titul.md"
    if not titul_path.exists():
        return {}
    
    try:
        raw = titul_path.read_text(encoding="utf-8-sig", errors="ignore").strip()
    except Exception:
        raw = titul_path.read_text(encoding="utf-8", errors="ignore").strip()
    
    if not raw:
        return {}
    
    meta: Dict[str, Any] = {}
    if raw.startswith("---"):
        parts = raw.split("---", 2)
        if len(parts) >= 3:
            try:
                data = yaml.safe_load(parts[1].strip())
                if isinstance(data, dict):
                    meta = data
            except Exception as e:
                logger.debug(f"Ошибка парсинга YAML в {titul_path}: {e}")
    
    if not meta:
        current_key = None
        current_value = []
        for line in raw.splitlines():
            line = line.strip()
            if not line:
                continue
            if ":" in line and not line.startswith((' ', '\t')):
                if current_key is not None and current_value:
                    meta[current_key] = ' '.join(current_value).strip()
                    current_value = []
                k, v = line.split(":", 1)
                current_key = k.strip()
                value_part = v.strip()
                if value_part:
                    current_value.append(value_part)
                else:
                    current_value = []
            elif current_key is not None:
                if line:
                    current_value.append(line)
        
        if current_key is not None and current_value:
            meta[current_key] = ' '.join(current_value).strip()
    
    meta = clean_metadata_values(meta)
    return meta

def merge_metadata(base: Dict[str, Any], titul: Dict[str, Any]) -> Dict[str, Any]:
    """Объединение метаданных без дублирования 'title'."""
    merged = {**base}
    for k, v in (titul or {}).items():
        if k != "title":
            merged[k] = v
    merged.pop("title", None)
    return merged

# ============================================================================
# ✅ ЯЧЕЙКА 10 — Улучшение MD-разметки (NEW)
# ============================================================================
def improve_markdown_formatting(text: str, title: str = "") -> str:
    """
    Улучшает Markdown-разметку документа:
    - Добавляет заголовки различных уровней (#, ##, ###, ####)
    - Структурирует разделы и подразделы
    - Убирает лишний мусор
    """
    if not text:
        return ""
    
    lines = text.splitlines()
    result_lines = []
    
    # Паттерны для обнаружения заголовков в тексте
    chapter_patterns = [
        (r'^(\d+)\s+([А-Я][а-яё]+.*)$', 1),  # "1 Название раздела"
        (r'^(\d+\.\d+)\s+([А-Я][а-яё]+.*)$', 2),  # "1.1 Подраздел"
        (r'^(\d+\.\d+\.\d+)\s+([А-Я][а-яё]+.*)$', 3),  # "1.1.1 Подподраздел"
        (r'^([IVX]+)\.\s+([А-Я][а-яё]+.*)$', 1),  # "I. Раздел"
        (r'^([А-Я][А-Я]+\.)\s+([А-Я][а-яё]+.*)$', 2),  # "РАЗДЕЛ 1"
    ]
    
    # Паттерны для терминов и определений
    term_pattern = r'^(\d+\.\d+\.\d+)\s+([а-яёА-ЯЁ][а-яёА-ЯЁ\s\-]+):\s*(.+)$'
    
    # Флаг для отслеживания текущего уровня
    in_section = False
    section_level = 0
    
    # Добавляем главный заголовок если есть title
    if title:
        result_lines.append(f"# {title}")
        result_lines.append("")
    
    for i, line in enumerate(lines):
        ln = line.strip()
        
        # Пропускаем пустые строки в начале
        if not ln and not result_lines:
            continue
        
        # Пропускаем служебные строки
        if any(x in ln.lower() for x in ['rowspan', 'colspan', 'x6.', 'x7.', 'x8.', 'x9.']):
            continue
        
        # Проверяем паттерны заголовков
        matched = False
        for pattern, level in chapter_patterns:
            match = re.match(pattern, ln, re.IGNORECASE)
            if match:
                # Определяем уровень заголовка
                if level == 1:
                    heading = "##"
                elif level == 2:
                    heading = "###"
                else:
                    heading = "####"
                
                # Формируем заголовок
                heading_text = match.group(2).strip()
                result_lines.append(f"{heading} {heading_text}")
                result_lines.append("")
                matched = True
                in_section = True
                section_level = level
                break
        
        if matched:
            continue
        
        # Проверяем термины и определения (нумерованный список с определениями)
        term_match = re.match(term_pattern, ln)
        if term_match:
            num = term_match.group(1)
            term_name = term_match.group(2).strip()
            term_def = term_match.group(3).strip()
            
            # Форматируем как список с выделением термина
            result_lines.append(f"- **{num} {term_name}**: {term_def}")
            result_lines.append("")
            continue
        
        # Обычные нумерованные пункты (3.1, 3.2, и т.д.)
        if re.match(r'^\d+\.\d+\s+', ln):
            result_lines.append(f"- {ln}")
            continue
        
        # Обычный текст - добавляем как есть
        if ln:
            result_lines.append(ln)
        else:
            # Сохраняем пустые строки для разделения абзацев
            if result_lines and result_lines[-1]:
                result_lines.append("")
    
    # Очищаем множественные пустые строки
    cleaned = []
    empty_count = 0
    for ln in result_lines:
        if not ln:
            empty_count += 1
            if empty_count <= 2:
                cleaned.append(ln)
        else:
            empty_count = 0
            cleaned.append(ln)
    
    return '\n'.join(cleaned).strip()

def write_formatted_markdown(out_path: Path, title: str, text: str):
    """
    Запись MD файла с улучшенной разметкой (БЕЗ YAML frontmatter).
    """
    out_path.parent.mkdir(parents=True, exist_ok=True)
    
    # Улучшаем разметку
    formatted_text = improve_markdown_formatting(text, title)
    
    # Записываем чистый MD без метаданных
    out_path.write_text(formatted_text, encoding="utf-8")
    logger.debug(f"Сохранён MD с улучшенной разметкой: {out_path}")

# ============================================================================
# ✅ ЯЧЕЙКА 11 — Обогащение + запись Markdown (Этап 1)
# ============================================================================
def enhance_metadata_nlp(text: str, meta: Dict[str, Any], cfg: PipelineConfig) -> Dict[str, Any]:
    """Обогащение метаданных ключевыми словами и сущностями."""
    meta["keywords"] = extract_keywords_advanced(text, max_keywords=cfg.max_keywords)
    meta["named_entities"] = extract_named_entities_spacy(text)
    struct_meta = detect_document_structure(text)
    meta.update(struct_meta)
    meta["document_id"] = generate_unique_document_id(meta)
    return meta

def filter_empty_metadata(meta: Dict[str, Any]) -> Dict[str, Any]:
    """Фильтрация метаданных."""
    excluded_fields = {
        "processed_at", "summary_status", "summary_model", "extractor",
        "summary_generated_at"
    }
    filtered = {}
    for k, v in meta.items():
        if k in excluded_fields:
            continue
        if v is None:
            continue
        if isinstance(v, str) and (not v.strip() or v.strip() in {"''", '""', "0"}):
            continue
        if isinstance(v, int) and v == 0 and k in {"pages", "page_count"}:
            continue
        if isinstance(v, (list, dict)) and not v:
            continue
        filtered[k] = v
    return filtered

def clean_md_body_text_for_storage(md_text: str) -> str:
    """Чистит ТОЛЬКО md-версию текста для хранения в outDOC/*.md"""
    if not md_text:
        return ""
    t = md_text
    t = re.sub(r'!\[[^\]]*\]\([^)]+\)', '', t)
    t = re.sub(r'<img\b[^>]*?/?>', '', t, flags=re.IGNORECASE)
    t = re.sub(r'<table\b[^>]*>.*?</table>', '', t, flags=re.DOTALL | re.IGNORECASE)
    t = re.sub(r'</?(tr|td|th)\b[^>]*>', '', t, flags=re.IGNORECASE)
    t = re.sub(r'</?(div|span|p|br)\b[^>]*>', '', t, flags=re.IGNORECASE)
    
    bad_line_patterns = [
        r'^\s*Wlld\)\w*.*$', r'^\s*Bx\.\s*N\s*$', r'^\s*вх\.\s*№\s*$',
        r'^\s*_{3,}\s*$', r'^\s*[-–]{3,}\s*$',
    ]
    
    lines = []
    for ln in t.splitlines():
        s = ln.strip()
        if not s:
            lines.append("")
            continue
        if any(re.match(pat, s, flags=re.IGNORECASE) for pat in bad_line_patterns):
            continue
        lines.append(ln.rstrip())
    
    t = "\n".join(lines)
    t = re.sub(r'\n{3,}', '\n', t).strip()
    return t

def write_markdown_stage1(out_path: Path, meta: Dict[str, Any], title: str, text: str, clean_text: str, cfg: PipelineConfig):
    """Запись на этапе 1: с YAML frontmatter и метаданными."""
    out_path.parent.mkdir(parents=True, exist_ok=True)
    
    filtered_meta = filter_empty_metadata(meta)
    doc_id = filtered_meta.pop("document_id", "")
    fm_ordered = {"document_id": doc_id}
    fm_ordered.update(filtered_meta)
    
    body_text = (text or "").strip()
    if getattr(cfg, "clean_md_text_on_write", True):
        body_text = clean_md_body_text_for_storage(body_text)
    
    parts = [
        "---",
        yaml.dump(fm_ordered, allow_unicode=True, sort_keys=False).strip(),
        "---\n"
    ]
    
    if title:
        parts.append(f"# {title}\n")
    
    parts.extend(["## Текст\n", body_text + "\n"])
    
    # Сохраняем clean_text для суммаризации
    clean_path = cfg.clean_text_dir / out_path.relative_to(cfg.output_dir).with_suffix(".txt")
    clean_path.parent.mkdir(parents=True, exist_ok=True)
    clean_path.write_text(clean_text or "", encoding="utf-8")
    logger.debug(f"Сохранён чистый текст для суммаризации: {clean_path}")
    
    out_path.write_text("\n".join(parts), encoding="utf-8")

# ============================================================================
# ✅ ЯЧЕЙКА 12 — Этап 1: Запуск извлечения
# ============================================================================
def process_one_file_stage1(file_path: Path, cfg: PipelineConfig, extractor: TextExtractor) -> Optional[Dict[str, Any]]:
    """Этап 1: извлечение текста через MinerU + создание улучшенного MD."""
    titul = load_titul_metadata(file_path.parent)
    text, _, meta_info = extractor.extract(file_path)
    
    if not text.strip():
        logger.warning(f"ПУСТОЙ ТЕКСТ: {file_path.name} (извлечено через {meta_info.get('extractor', 'unknown')})")
        return None
    
    base_meta = {
        "source_path": str(file_path.resolve()),
        "original_filename": file_path.name,
        "pages": meta_info.get("pages", 0),
    }
    
    meta = merge_metadata(base_meta, titul)
    meta = enhance_metadata_nlp(text, meta, cfg)
    clean_text = extract_clean_text_for_summary(text)
    
    rel = file_path.relative_to(cfg.input_dir)
    
    # === Путь для MD с метаданными (outDOC) ===
    md_path = cfg.output_dir / rel.with_suffix(".md")
    title = meta.get("наименование_документа") or meta.get("document_id") or file_path.stem
    write_markdown_stage1(md_path, meta, title=title, text=text, clean_text=clean_text, cfg=cfg)
    
    # === NEW: Путь для MD с улучшенной разметкой (outDOCmd) ===
    if getattr(cfg, "run_formatted_md", True):
        formatted_md_path = cfg.formatted_md_dir / rel.with_suffix(".md")
        write_formatted_markdown(formatted_md_path, title=title, text=text)
    
    return {
        "file_path": str(file_path),
        "markdown_path": str(md_path),
        "formatted_md_path": str(formatted_md_path) if getattr(cfg, "run_formatted_md", True) else None,
        "clean_text_saved": True
    }

def run_stage1(cfg: PipelineConfig) -> List[Dict[str, Any]]:
    """Запуск этапа 1: извлечение текста + улучшенная MD-разметка."""
    logger.info("=== ЭТАП 1: ИЗВЛЕЧЕНИЕ ТЕКСТА ЧЕРЕЗ MINERU ===")
    
    cfg.output_dir.mkdir(parents=True, exist_ok=True)
    cfg.clean_text_dir.mkdir(parents=True, exist_ok=True)
    
    # NEW: Создаём каталог для улучшенной MD-разметки
    if getattr(cfg, "run_formatted_md", True):
        cfg.formatted_md_dir.mkdir(parents=True, exist_ok=True)
        logger.info(f"Каталог для улучшенной MD: {cfg.formatted_md_dir}")
    
    mineru = MinerUClient(cfg.mineru_url)
    extractor = TextExtractor(cfg, mineru=mineru)
    
    results = []
    files_to_process = [
        p for p in cfg.input_dir.rglob("*")
        if p.is_file()
        and p.suffix.lower() in {".pdf", ".docx", ".txt", ".md"}
        and p.name != "titul.md"
    ]
    
    logger.info(f"Найдено файлов для обработки: {len(files_to_process)}")
    
    for p in files_to_process:
        try:
            if (r := process_one_file_stage1(p, cfg, extractor)):
                results.append(r)
                logger.info(f"OK: {p.name} -> {Path(r['markdown_path']).name}")
                if r.get('formatted_md_path'):
                    logger.info(f"     Форматированный MD: {Path(r['formatted_md_path']).name}")
        except Exception as e:
            logger.exception(f"ОШИБКА: {p.name} | {e}")
    
    logger.info(f"Этап 1 завершён. Обработано файлов: {len(results)}")
    return results

# ============================================================================
# ✅ ЯЧЕЙКА 13 — Pydantic схемы для структурированного резюме
# ============================================================================
from typing import Optional, List
from pydantic import BaseModel, Field

class Entity(BaseModel):
    """Сущность, извлечённая из документа."""
    name: str = Field(description="Имя сущности")
    type: str = Field(description="Тип: person, organization, date, location, technology, other")
    context: Optional[str] = Field(default=None, description="Краткий контекст упоминания")

class ActionItem(BaseModel):
    """Задача или действие, выявленное в документе."""
    task: str = Field(description="Действие, которое необходимо выполнить")
    owner: Optional[str] = Field(default=None, description="Исполнитель (если указан)")
    deadline: Optional[str] = Field(default=None, description="Срок выполнения (если указан)")
    priority: Optional[str] = Field(default=None, description="Приоритет: high, medium, low")

class DocumentSummary(BaseModel):
    """Структурированное резюме документа."""
    title: str = Field(description="Заголовок документа или выведенная тема")
    document_type: str = Field(description="Тип: report, article, meeting_notes, research_paper, email, other")
    summary: str = Field(description="Краткое резюме в 1-3 абзацах")
    key_points: List[str] = Field(description="3-7 ключевых тезисов в виде списка")
    entities: List[Entity] = Field(default_factory=list, description="Ключевые сущности")
    action_items: List[ActionItem] = Field(default_factory=list, description="Выявленные задачи и следующие шаги")
    word_count: int = Field(description="Количество слов в исходном документе")
    confidence: float = Field(description="Уверенность в точности резюме (0.0-1.0)", ge=0.0, le=1.0)

# ============================================================================
# ✅ ЯЧЕЙКА 14 — Инструмент чтения текстовых файлов
# ============================================================================
def read_text_file(file_path: str) -> str:
    """Чтение содержимого текстового файла для Agno Agent."""
    path = Path(file_path)
    if not path.exists():
        return f"Error: File not found: {file_path}"
    
    supported_extensions = {".txt", ".md", ".markdown", ".rst", ".text"}
    if path.suffix.lower() not in supported_extensions:
        return f"Error: Unsupported file type: {path.suffix}"
    
    try:
        encodings = ["utf-8", "utf-8-sig", "latin-1", "cp1252"]
        content = None
        for encoding in encodings:
            try:
                content = path.read_text(encoding=encoding)
                break
            except UnicodeDecodeError:
                continue
        
        if content is None:
            return "Error: Could not decode file with supported encodings"
        
        logger.info(f"Read text file: {path.name} ({len(content)} chars)")
        word_count = len(content.split())
        metadata = f"File: {path.name}\nType: {path.suffix}\nWords: {word_count}"
        return f"{metadata}\n{content}"
    except Exception as e:
        logger.error(f"Error reading text file {file_path}: {e}")
        return f"Error reading file: {e}"

# ============================================================================
# ✅ ЯЧЕЙКА 15 — Настройка Agno Agent (ФИКС РУССКОГО ЯЗЫКА)
# ============================================================================
import sys
from agno.agent import Agent
from agno.db.sqlite import SqliteDb
from agno.models.ollama import Ollama
from agno.tools.reasoning import ReasoningTools
from agno.culture.manager import CultureManager
from agno.db.schemas.culture import CulturalKnowledge

# База данных для хранения истории агента и метаданных
agent_db = SqliteDb(db_file="tmp/agents.db")

# Добавление культурных знаний (инструкция на русском)
culture_manager = CultureManager(db=agent_db)
document_search_culture = CulturalKnowledge(
    name="Инструкция для агента суммаризации промышленных документов",
    summary="Правила анализа и краткого изложения технической, отчётной и регламентной документации",
    categories=["summarization", "industrial_docs", "language", "ux"],
    content="""Вы — эксперт по анализу производственной документации. Ваша задача — суммировать технические, отчётные и регламентные документы компании.

## 🚫 КРИТИЧЕСКИ ВАЖНО — ЯЗЫК ОТВЕТА:
- **ВСЕГДА отвечайте ТОЛЬКО на РУССКОМ языке**
- **НИКОГДА не переходите на английский**, даже если встретите английские термины в документе
- Технические аббревиатуры (ГОСТ, СТО, ТУ) оставляйте как есть
- Если сомневаетесь — всё равно пишите на русском

## КРАТКОСТЬ И ФОРМАТ ОТВЕТА:
1. **Будьте лаконичны, но исчерпывающи** — отвечайте по существу, без воды
2. **Структура ответа**:
   - 📌 **Краткое резюме** (2–4 предложения) — в начале
   - 🔑 **Ключевые тезисы** — маркированный список (3–7 пунктов)
   - ⚙️ **Извлечённые сущности** — оборудование, материалы, стандарты (ГОСТ, ТУ), подразделения
   - ✅ **Задачи и действия** — с указанием: тип работы, объект, исполнитель, срок (если есть)
3. **Ограничения**:
   - Общий ответ: не более 600 слов
   - Цитаты — только при необходимости, с указанием источника

## ТЕХНИЧЕСКАЯ ТОЧНОСТЬ:
- Сохраняйте профессиональную терминологию, не упрощайте технические понятия
- Не добавляйте информацию, отсутствующую в исходном документе
- При неоднозначности указывайте уровень уверенности

## ИНСТРУМЕНТЫ И ПОДХОД:
- Перед суммаризацией используйте инструмент рассуждения (`think`) для планирования структуры ответа
- При работе с большими документами — суммируйте по разделам, затем объединяйте
- Если документ содержит таблицы/схемы — опишите их смысл текстом

## ВАЖНО:
- ❌ НИКОГДА не выдумывайте данные, цифры или требования
- ❌ Не интерпретируйте нормативные ссылки без прямого указания в тексте
- ✅ Если информации недостаточно — честно сообщите об этом
- ✅ При обнаружении противоречий в документе — укажите на это явно
""",
    notes=[
        "Адаптировано для русскоязычных промышленных документов",
        "Приоритет: техническая точность > краткость > полнота",
        "Поддержка стандартов: ГОСТ, ТУ, СТО, РД",
        "СТРОГО РУССКИЙ ЯЗЫК — без исключений"
    ],
    metadata={
        "source": "industrial_summarization_policy",
        "language": "ru",
        "version": 1,
        "doc_types": ["technical_reports", "regulations", "maintenance_logs", "specifications"]
    },
)
culture_manager.add_cultural_knowledge(document_search_culture)
print("✅ Культурные знания добавлены")

# Определение пути к директории для сохранения БД
try:
    notebook_dir = Path(globals()['_dh'][0])
except (KeyError, NameError):
    notebook_dir = Path.cwd()

db_dir = notebook_dir / "tmp"
db_dir.mkdir(exist_ok=True)

SYSTEM_MESSAGE = """\
Вы — эксперт по анализу производственной документации. Ваша задача — суммировать технические, отчётные и регламентные документы.

## 🚫 КРИТИЧЕСКИ ВАЖНО — ЯЗЫК:
- **ОТВЕЧАЙТЕ ТОЛЬКО НА РУССКОМ ЯЗЫКЕ**
- **ЗАПРЕЩЕНО переходить на английский** — даже для технических терминов
- Все поля в ответе (title, summary, key_points, entities, action_items) — ТОЛЬКО на русском

## Ваши обязанности
1. **Суммировать** — Создать краткое, точное и технически корректное резюме документа.
2. **Извлечь ключевые пункты** — Выделить 3–7 наиболее важных тезисов.
3. **Определить сущности** — Извлечь и классифицировать: оборудование, материалы, стандарты (ГОСТ, ТУ), подразделения, должности.
4. **Найти задачи** — Выявить явные задачи, указав: тип работы, объект/участок, исполнителя, срок.

## Руководящие принципы
- **Сохраняйте техническую точность**. Не упрощайте термины.
- **Будьте краткими, но исчерпывающими**.
- **Используйте ясный, профессиональный язык**.
- **ОТВЕЧАЙ НА РУССКОМ ЯЗЫКЕ** — это требование системы.
- **Не добавляйте информацию**, отсутствующую в исходном документе.
- **Уровень уверенности**: 0.9-1.0 (чёткий документ), 0.7-0.9 (понятен), 0.5-0.7 (неясность), <0.5 (невозможно).

Используйте инструмент рассуждения (`think`), чтобы спланировать подход перед суммаризацией.
"""

# Создание агента
summarizer_agent = Agent(
    name="Production Document Summarizer",
    model=Ollama(id="qwen3:14b", host="http://172.18.12.45:11434"),
    system_message=SYSTEM_MESSAGE,
    add_culture_to_context=True,
    output_schema=DocumentSummary,
    tools=[
        ReasoningTools(add_instructions=True),
        read_text_file,
    ],
    add_datetime_to_context=True,
    add_history_to_context=True,
    num_history_runs=5,
    enable_agentic_memory=True,
    markdown=True,
    db=agent_db,
    telemetry=False,
    instructions=[
        "Всегда отвечай на русском языке",
        "Не используй английский в ответе",
        "Все поля схемы должны быть заполнены на русском"
    ]
)

# ============================================================================
# ✅ ЯЧЕЙКА 16 — Функции суммаризации
# ============================================================================
def summarize_text_file(file_path: str) -> DocumentSummary:
    """Суммирование одного текстового файла через Agno Agent."""
    try:
        content = read_text_file(file_path)
        if content.startswith("Error:"):
            raise ValueError(content)
        
        response = summarizer_agent.run(
            f"Пожалуйста, суммируйте следующий текстовый документ (ОТВЕТ ТОЛЬКО НА РУССКОМ):\n{content}"
        )
        
        if response.content and isinstance(response.content, DocumentSummary):
            return response.content
        else:
            raise ValueError("Не удалось сгенерировать резюме")
    except Exception as e:
        raise ValueError(f"Ошибка при обработке {file_path}: {e}")

# ============================================================================
# ✅ ЯЧЕЙКА 17 — Обновление Markdown с структурированным резюме
# ============================================================================
def update_markdown_with_structured_summary(md_path: Path, summary: DocumentSummary, model: str):
    """Обновляет существующий Markdown с структурированным резюме от Agno Agent."""
    content = md_path.read_text(encoding="utf-8", errors="ignore")
    
    # --- 1) Парсим YAML фронтматтер ---
    fm = {}
    body = content
    if content.startswith("---"):
        parts = content.split("---", 2)
        if len(parts) >= 3:
            fm_raw = parts[1].strip()
            body = parts[2].lstrip("\n")
            try:
                fm = yaml.safe_load(fm_raw) or {}
            except Exception:
                fm = {}
    
    # --- 2) Обновляем метаданные суммаризации ---
    doc_id = fm.pop("document_id", "")
    fm["summary_generated_at"] = datetime.now().isoformat()
    fm_ordered = {"document_id": doc_id}
    fm_ordered.update(fm)
    
    # --- 3) Формируем блок резюме ---
    if summary:
        summary_block = f"\n{summary.summary}\n\n### Ключевые пункты\n"
        for i, point in enumerate(summary.key_points, 1):
            summary_block += f"{i}. {point}\n"
        
        if summary.entities:
            summary_block += "\n### Сущности\n"
            for ent in summary.entities[:10]:
                summary_block += f"- **{ent.name}** ({ent.type})\n"
        
        if summary.action_items:
            summary_block += "\n### Задачи\n"
            for item in summary.action_items:
                summary_block += f"- {item.task}"
                if item.owner:
                    summary_block += f" (Ответственный: {item.owner})"
                if item.deadline:
                    summary_block += f" [Срок: {item.deadline}]"
                summary_block += "\n"
        
        summary_block += f"\n_Уверенность: {summary.confidence}_\n"
    else:
        summary_block = "_Суммаризация не удалась_\n"
    
    # --- 4) Вставка/замена секции '## Краткое содержание' ---
    sec_re = re.compile(r'(^##\s*Краткое содержание\s*\n)(.*?)(?=^\s*##\s|\Z)', re.DOTALL | re.MULTILINE)
    if sec_re.search(body):
        def _repl(m):
            return m.group(1) + "\n" + summary_block + "\n"
        
        body_new = sec_re.sub(_repl, body, count=1)
        matches = list(sec_re.finditer(body_new))
        if len(matches) > 1:
            first = matches[0]
            keep_end = first.end()
            after = body_new[keep_end:]
            after = sec_re.sub("", after)
            body_new = body_new[:keep_end] + after
        body = body_new.strip() + "\n"
    else:
        lines = body.splitlines()
        out_lines = []
        inserted = False
        for i, line in enumerate(lines):
            out_lines.append(line)
            if (not inserted) and line.startswith("# "):
                out_lines.append("")
                out_lines.append("## Краткое содержание")
                out_lines.append("")
                out_lines.append(summary_block.rstrip("\n"))
                out_lines.append("")
                inserted = True
        
        if not inserted:
            body = "## Краткое содержание\n" + summary_block + "\n" + body.strip() + "\n"
        else:
            body = "\n".join(out_lines).strip() + "\n"
    
    # --- 5) Собираем финальный файл ---
    new_content_parts = [
        "---",
        yaml.dump(fm_ordered, allow_unicode=True, sort_keys=False).strip(),
        "---\n",
        body
    ]
    md_path.write_text("\n".join(new_content_parts), encoding="utf-8")
    logger.info(f"Обновлён файл с кратким содержанием: {md_path.name}")

# ============================================================================
# ✅ ЯЧЕЙКА 18 — Этап 2: Запуск суммаризации
# ============================================================================
def run_stage2(cfg: PipelineConfig) -> List[Dict[str, Any]]:
    """Запуск этапа 2: суммаризация через Agno Agent."""
    logger.info("=== ЭТАП 2: СУММАРИЗАЦИЯ ЧЕРЕЗ AGNO AGENT ===")
    
    # Проверка подключения к Ollama
    try:
        models_response = ollama.list()
        if isinstance(models_response, dict) and 'models' in models_response:
            model_names = [m.get('name', '').split(':')[0] for m in models_response['models'] if isinstance(m, dict)]
        else:
            model_names = [str(m).split(':')[0] for m in models_response]
        
        target_model_base = cfg.ollama_summary_model.split(':')[0]
        if target_model_base not in model_names:
            logger.warning(f"Модель {cfg.ollama_summary_model} не найдена. Доступные модели: {model_names}")
        
        if not model_names:
            logger.error("Ollama запущен, но нет загруженных моделей. Загрузите модель: ollama pull qwen3:14b")
            return []
        
        logger.info(f"Ollama доступен. Используем модель: {cfg.ollama_summary_model}")
    except Exception as e:
        logger.error(f"Не удалось подключиться к Ollama: {e}")
        logger.error("Убедитесь, что Ollama запущен: ollama serve")
        return []
    
    md_files = list(cfg.output_dir.rglob("*.md"))
    logger.info(f"Найдено Markdown файлов для суммаризации: {len(md_files)}")
    
    results: List[Dict[str, Any]] = []
    for md_path in md_files:
        try:
            content = md_path.read_text(encoding="utf-8", errors="ignore")
            
            # Пропускаем, если уже completed
            if '"summary_generated_at"' in content or 'summary_generated_at:' in content:
                logger.debug(f"Пропускаем (уже обработан): {md_path.name}")
                continue
            
            # Читаем чистый текст из outDOC_clean
            rel = md_path.relative_to(cfg.output_dir)
            clean_path = (cfg.clean_text_dir / rel).with_suffix(".txt")
            
            if not clean_path.exists():
                logger.warning(f"Не найден clean-text файл (пропуск суммаризации): {clean_path}")
                update_markdown_with_structured_summary(md_path, None, cfg.ollama_summary_model)
                results.append({"file": md_path.name, "status": "failed", "error": "missing_clean_text"})
                continue
            
            doc_text_clean = clean_path.read_text(encoding="utf-8", errors="ignore").strip()
            if not doc_text_clean:
                logger.warning(f"Пустой clean-text файл: {clean_path}")
                update_markdown_with_structured_summary(md_path, None, cfg.ollama_summary_model)
                results.append({"file": md_path.name, "status": "failed", "error": "empty_clean_text"})
                continue
            
            # Генерация структурированного резюме через Agno
            summary = summarize_text_file(str(clean_path))
            
            # Обновляем Markdown
            update_markdown_with_structured_summary(md_path, summary, cfg.ollama_summary_model)
            
            results.append({
                "file": md_path.name,
                "status": "completed" if summary else "failed",
                "confidence": summary.confidence if summary else 0
            })
            time.sleep(1.5)
        except Exception as e:
            logger.exception(f"Ошибка суммаризации файла {md_path.name}: {e}")
            results.append({"file": md_path.name, "status": "error", "error": str(e)})
    
    logger.info(f"Этап 2 завершён. Обработано файлов: {len(results)}")
    return results

# ============================================================================
# ✅ ЯЧЕЙКА 19 — Запуск пайплайна: оцифровка (Этап 1)
# ============================================================================
if __name__ == '__main__':
    cfg = PipelineConfig(
        input_dir=Path("InDOC"),
        output_dir=Path("outDOC"),
        clean_text_dir=Path("outDOC_clean"),
        formatted_md_dir=Path("outDOCmd"),  # NEW
        run_extraction=True,
        run_summarization=False,
        run_formatted_md=True,  # NEW: включено по умолчанию
        ollama_summary_model="qwen3:14b",
        ollama_host="http://localhost:11434",
    )
    
    results = []
    
    # ===================== ЭТАП 1 =====================
    if cfg.run_extraction:
        logger.info("Запуск ЭТАПА 1 (извлечение текста через MinerU). Суммаризация НЕ выполняется.")
        results = run_stage1(cfg)
        print(f"\nЭтап 1 завершён. Обработано файлов: {len(results)}")
        
        # Статистика по ключевым словам
        if results:
            sample_file = cfg.output_dir / Path(results[0]["markdown_path"]).relative_to(cfg.output_dir)
            if sample_file.exists():
                try:
                    content = sample_file.read_text(encoding="utf-8")
                    fm_part = content.split("---")[1].strip()
                    fm = yaml.safe_load(fm_part)
                    print(f"\nПример метаданных из первого файла:")
                    print(f"  document_id: {fm.get('document_id', 'N/A')}")
                    print(f"  Пример ключевых слов:")
                    for i, kw in enumerate(fm.get("keywords", [])[:5], 1):
                        print(f"    {i}. {kw}")
                except Exception as e:
                    logger.warning(f"Не удалось прочитать метаданные: {e}")
    
    # ===================== ЭТАП 2 =====================
    if cfg.run_summarization:
        logger.info("\nЗапуск ЭТАПА 2 (суммаризация через Agno Agent).")
        results = run_stage2(cfg)
        print(f"\nЭтап 2 завершён. Результаты суммаризации:")
        for r in results[:5]:
            status = "✅" if r["status"] == "completed" else "❌"
            conf = f"(conf: {r.get('confidence', 0):.2f})" if r.get('confidence') else ""
            print(f"  {status} {r['file']}: {r['status']} {conf} {('(' + r['error'] + ')') if r.get('error') else ''}")
    
    if not cfg.run_extraction and not cfg.run_summarization:
        print("Ни один этап не запущен. Установите флаги run_extraction=True или run_summarization=True в конфигурации.")

# ============================================================================
# ✅ ЯЧЕЙКА 20 — Запуск пайплайна: оцифровка (Этап 1)
# ============================================================================
if __name__ == '__main__':
    cfg = PipelineConfig(
        input_dir=Path("InDOC"),
        output_dir=Path("outDOC"),
        clean_text_dir=Path("outDOC_clean"),
        formatted_md_dir=Path("outDOCmd"),  # NEW
        run_extraction=False,
        run_summarization=True,
        run_formatted_md=False,  # NEW: включено по умолчанию
        ollama_summary_model="qwen3:14b",
        ollama_host="http://localhost:11434",
    )
    
    results = []
    
    # ===================== ЭТАП 1 =====================
    if cfg.run_extraction:
        logger.info("Запуск ЭТАПА 1 (извлечение текста через MinerU). Суммаризация НЕ выполняется.")
        results = run_stage1(cfg)
        print(f"\nЭтап 1 завершён. Обработано файлов: {len(results)}")
        
        # Статистика по ключевым словам
        if results:
            sample_file = cfg.output_dir / Path(results[0]["markdown_path"]).relative_to(cfg.output_dir)
            if sample_file.exists():
                try:
                    content = sample_file.read_text(encoding="utf-8")
                    fm_part = content.split("---")[1].strip()
                    fm = yaml.safe_load(fm_part)
                    print(f"\nПример метаданных из первого файла:")
                    print(f"  document_id: {fm.get('document_id', 'N/A')}")
                    print(f"  Пример ключевых слов:")
                    for i, kw in enumerate(fm.get("keywords", [])[:5], 1):
                        print(f"    {i}. {kw}")
                except Exception as e:
                    logger.warning(f"Не удалось прочитать метаданные: {e}")
    
    # ===================== ЭТАП 2 =====================
    if cfg.run_summarization:
        logger.info("\nЗапуск ЭТАПА 2 (суммаризация через Agno Agent).")
        results = run_stage2(cfg)
        print(f"\nЭтап 2 завершён. Результаты суммаризации:")
        for r in results[:5]:
            status = "✅" if r["status"] == "completed" else "❌"
            conf = f"(conf: {r.get('confidence', 0):.2f})" if r.get('confidence') else ""
            print(f"  {status} {r['file']}: {r['status']} {conf} {('(' + r['error'] + ')') if r.get('error') else ''}")
    
    if not cfg.run_extraction and not cfg.run_summarization:
        print("Ни один этап не запущен. Установите флаги run_extraction=True или run_summarization=True в конфигурации.")		
```

## 📁 Структура выходных файлов

После запуска пайплайна вы получите **три типа выходных файлов**:

| Каталог | Формат | Содержимое |
|---------|--------|------------|
| `outDOC/` | `.md` | С YAML frontmatter (метаданные, keywords, entities) |
| `outDOC_clean/` | `.txt` | Чистый текст для суммаризации (без мусора) |
| `outDOCmd/` | `.md` | **Улучшенная разметка** (заголовки #, ##, ###, списки) **без метаданных** |

## 🔑 Ключевые изменения

1. **Новый конфиг** `formatted_md_dir: Path("outDOCmd")`
2. **Новый флаг** `run_formatted_md: bool = True`
3. **Функция** `improve_markdown_formatting()` — распознаёт заголовки и структурирует документ
4. **Функция** `write_formatted_markdown()` — пишет MD без YAML frontmatter
5. **Интеграция в Stage 1** — создаётся сразу после OCR через MinerU